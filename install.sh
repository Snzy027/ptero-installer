...
preflight(){
    ...
    output "Copyright © 2021 Sneezy Junior"
...
install_pterodactyl() {
    ...
    if [ ${PANEL} = "latest" ]; then
    	curl -Lo panel.tar.gz https://github.com/pterodactyl/panel/releases/tag/v1.6.3/download/panel.tar.gz
    else
...
EOF
	...
	output "Since you are on a system with targeted SELinux policies, you should be changing the Daemon Server File Directory from /var/lib/pterodactyl/volumes to /var/srv/containers/pterodactyl."
	output "------------------------------------------------------------------"
    fi
}
...
upgrade_wings(){
    if [ ${WINGS} = "latest" ]; then
    	curl -L -o /usr/local/bin/wings https://github.com/pterodactyl/wings/releases/tag/${WINGS}/download/wings_linux_amd64
    else
    	curl -L -o /usr/local/bin/wings https://github.com/pterodactyl/wings/releases/download/${WINGS}/wings_linux_amd64
    fi
    chmod u+x /usr/local/bin/wings
